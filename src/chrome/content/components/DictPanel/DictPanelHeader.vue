<template>
  <div class="ll-dict-crx-content-header">
    <!-- 查询header -->
    <div>{{ dictModel.query }}</div>
    <div class="ll-dict-crx-content-header__voice-btn" @click="palyAudio" />
    <!-- 中文含义 -->
    <div>{{ dictModel.translation }}</div>
    <!-- 音标 -->
    <div>
      英
      <span>[{{ dictModel.ukPhonetic }}]</span>
      美
      <span>[{{ dictModel.usPhonetic }}]</span>
    </div>
  </div>
</template>
